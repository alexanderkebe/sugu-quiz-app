        ...
        <div className="w-full max-w-3xl mb-4 sm:mb-6">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-2 sm:gap-3 mb-3 sm:mb-0">
            ...
            <motion.div
              ...
              initial={{ scale: 1.2 }}
              animate={{ scale: 1 }}
              className={`font-nokia font-bold text-lg sm:text-xl md:text-2xl px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg order-3 sm:order-none ${timeLeft <= 10 ? 'text-red-400' : timeLeft <= 15 ? 'text-yellow-400' : 'text-gold'
                }`}
              style={{
                background: 'rgba(255, 255, 255, 0.1)',
                backdropFilter: 'blur(10px)',
            ...
            {question.options.map((option, index) => {
              ...
              return (
                <motion.button
                  ...
                  disabled={isLocked}
                  className={`w-full text-left font-nokia text-sm sm:text-base md:text-lg py-3 sm:py-4 md:py-5 px-4 sm:px-6 md:px-8 rounded-xl sm:rounded-2xl transition-all duration-300 min-h-[44px] sm:min-h-[52px] ${isLocked ? 'cursor-not-allowed' : 'cursor-pointer'
                    }`}
                  style={{
                    background:
